...
```
```` ``` ````
**Reference:** cudf.md memory-stream section: "NOT safe: std::vector as source for async copy without stream.synchronize() before destruction (pageable memory is not stream-ordered)". Verified the code at src/main/cpp/src/protobuf/protobuf.cu:1026-1060 — no stream.synchronize() before h_scan_descs leaves scope.
...
```
```` ``` ````
**Reference:** Sibling new API at src/main/cpp/src/cast_string.hpp:202 uses `` `cudf::get_current_device_resource_ref()` ``; conventions.md Section memory-stream ("`` `rmm::mr::get_current_device_resource_ref()` `` is DEPRECATED — MUST use `` `cudf::get_current_device_resource_ref()` ``"). Note: I verified at /mnt/nvme/home/haoyangl/cc_exp/spark-rapids-jni/target/libcudf-install/include/rmm/mr/per_device_resource.hpp:187 that the rmm function carries no `` `[[deprecated]]` `` attribute in the pinned rmm version, and that this `` `rmm::mr::` `` form is used widely elsewhere in the existing codebase (e.g. cast_string.hpp:82-143, row_conversion.hpp:34-52). Hence this is a consistency concern (SHOULD FIX), not a compiler-enforced deprecation.
...
```
```` ``` ````
**Reference:** Convention (memory-stream section): "rmm::mr::get_current_device_resource_ref() is DEPRECATED — MUST use cudf::get_current_device_resource_ref() (MUST FIX)". The cudf wrapper is defined at thirdparty/cudf/cpp/include/cudf/utilities/memory_resource.hpp:27 and is already included via charset_decode.hpp:22. Other new functions in the same PR use the correct form, e.g., src/main/cpp/src/cast_string.hpp:234.
*(Reviewer: jni-boundary)*
...
```
```` ``` ````
**Reference:** Convention "Section: gpu-kernel-correctness > cuda::std:: Mandate" in .review/conventions.md:639-678 (trait substitutions); `` `#include <cuda/std/type_traits>` `` already present at src/main/cpp/src/protobuf/protobuf_device_helpers.cuh:27.
...
#### 6. [🟢 SUGGESTION] **Outside review scope:** The `` `@return` `` tag says "7 columns" but the `` `@brief` `` immediately above (line 147) says "6 children," and the implementation at `` `src/main/cpp/src/cast_string_to_datetime.cu:934-940` `` pushes exactly 6 children into `` `output_columns` `` via 6 `` `emplace_back` `` calls

**Line:** 108
**Issue:** **Outside review scope:** The `` `@return` `` tag says "7 columns" but the `` `@brief` `` immediately above (line 147) says "6 children," and the implementation at `` `src/main/cpp/src/cast_string_to_datetime.cu:934-940` `` pushes exactly 6 children into `` `output_columns` `` via 6 `` `emplace_back` `` calls. The count "7" in `` `@return` `` is stale, making the documentation internally contradictory.
**Confidence:** 🟣❗ CERTAIN
**Code:**
```` ```cpp ````
```cpp
...
```
```` ``` ````
**Reference:** src/main/cpp/src/cast_string_to_datetime.cu:934-940 (6 emplace_back calls); src/main/cpp/src/cast_string.hpp:147 (@brief says "6 children")
...
```
```` ``` ````
**Reference:** src/main/cpp/src/protobuf/protobuf_builders.cu:162 — `` `cudf::detail::make_device_uvector_async` `` already used for exactly the same upload pattern.
*(Reviewer: refactor)*
...
```
```` ``` ````
**Reference:** `` `build_lookup_table` `` and `` `build_index_lookup_table` `` already centralised at src/main/cpp/src/protobuf/protobuf_host_helpers.hpp:48-73; the upload counterpart is currently missing, causing the repeated boilerplate in protobuf.cu:563-606, 633-642, 1035-1050.
*(Reviewer: refactor)*
...
```
```` ``` ````
**Reference:** `` `cudf::detail::make_device_uvector_async` `` used at src/main/cpp/src/protobuf/protobuf_builders.cu:162,185,190; raw pattern repeated at protobuf.cu:549-588, 624-641, 784-789, 1029-1049.
*(Reviewer: refactor)*
...
```
```` ``` ````
**Reference:** Zeroing pattern with `` `thrust::fill` `` used in `` `make_null_list_column_with_child` `` at src/main/cpp/src/protobuf/protobuf_builders.cu:120; `` `cudaMemsetAsync` `` variant at protobuf_builders.cu:97-98, 140-141.
*(Reviewer: refactor)*
...
```
```` ``` ````
**Reference:** src/main/cpp/src/cast_string.hpp:196 — uses `` `cudf::get_current_device_resource_ref()` `` in the newly-added `` `parse_timestamp_strings_with_format` `` declaration; cudf memory-stream conventions: "MUST use `` `cudf::get_current_device_resource_ref()` `` (MUST FIX)"
*(Reviewer: compliant)*
...
```
```` ``` ````
**Reference:** `` `validate_enum_and_propagate_rows` `` declared at src/main/cpp/src/protobuf/protobuf_host_helpers.hpp:168-175; duplicate sequence at src/main/cpp/src/protobuf/protobuf_builders.cu:252-271.
*(Reviewer: refactor)*
...
```
```` ``` ````
**Reference:** src/main/cpp/src/cast_string.hpp:234 — new `` `parse_timestamp_strings_with_format` `` function added in this PR uses `` `cudf::get_current_device_resource_ref()` ``. The memory-stream convention (cudf.md Section: memory-stream) states: "`` `rmm::mr::get_current_device_resource_ref()` `` is DEPRECATED — MUST use `` `cudf::get_current_device_resource_ref()` `` (MUST FIX)".
*(Reviewer: compliant)*
...
```
```` ``` ````
**Reference:** `` `cudf::make_empty_column` `` / `` `make_empty_column_safe` `` used at src/main/cpp/src/protobuf/protobuf_builders.cu:109; `` `cudf::make_empty_column()` `` mentioned in conventions Section: refactor under "cuDF Reusable Utilities".
*(Reviewer: refactor)*
...
```
```` ``` ````
**Reference:** src/main/cpp/src/protobuf/protobuf_builders.cu:430-431 ("Transfer ownership of the chars buffer instead of copying — the strings path below uses `` `chars.release()` `` for the same reason."); same-function STRING branch at src/main/cpp/src/protobuf/protobuf_kernels.cuh:681 uses `` `chars.release()` ``.
*(Reviewer: memory-stream)*
...
```
```` ``` ````
**Reference:** src/main/cpp/src/protobuf/protobuf_builders.cu:24-42 — `` `make_list_column_with_input_nulls` `` provides identical logic. Already used at protobuf_builders.cu:352 and :446.
*(Reviewer: refactor)*
...
```
```` ``` ````
**Reference:** src/main/cpp/src/protobuf/protobuf_kernels.cuh:434-435 (CUDF_EXPECTS guard that is exercised here)
...
```
```` ``` ````
**Reference:** conventions.md:614 ("NOT safe: `` `std::vector` `` as source for async copy without `` `stream.synchronize()` `` before destruction"); CUDA Runtime API sync-behavior doc (https://docs.nvidia.com/cuda/cuda-runtime-api/api-sync-behavior.html): "For transfers between device memory and pageable host memory, the function might be synchronous with respect to host"; in-PR correct pattern at src/main/cpp/src/protobuf/protobuf_builders.cu:162.
*(Reviewer: correctness)*
...
```
```` ``` ````
**Reference:** src/main/cpp/src/parse_timestamp_with_format.cu:368-381
*(Reviewer: test-coverage)*
...
```
```` ``` ````
**Reference:** src/test/java/com/nvidia/spark/rapids/jni/ProtobufTest.java:2843-2849 (testNullInputRowProducesNullStructRow verifies nullness on the struct row level — analogous pattern for field nullness).
*(Reviewer: test-coverage)*
...
```
```` ``` ````
**Reference:** src/main/cpp/src/protobuf/protobuf.cu:1003-1008 (exclusive_scan builds per-row LIST offsets)
*(Reviewer: test-coverage)*
...
```
```` ``` ````
**Reference:** src/test/java/com/nvidia/spark/rapids/jni/ProtobufTest.java:3003-3007 (verifyDeepNesting reads and asserts the leaf value)
*(Reviewer: test-coverage)*
...
```
```` ``` ````
**Reference:** src/test/java/com/nvidia/spark/rapids/jni/ProtobufTest.java:2936-2941 (testLargeFieldNumber correctly asserts the decoded integer value after drilling into the child).
*(Reviewer: test-coverage)*
...
```
```` ``` ````
**Reference:** src/test/java/com/nvidia/spark/rapids/jni/ProtobufTest.java:1874-1876 (comment identifying the gap)
*(Reviewer: test-coverage)*
...
```
```` ``` ````
**Reference:** src/test/java/com/nvidia/spark/rapids/jni/ProtobufTest.java:2945-3009 (method body with assertions); the analogous single-level nested decode is exercised by testNestedMessage at line 1909.
*(Reviewer: test-coverage)*
