...
void nvtxwConsumeConfigString(char const* config, nvtxwKeyValuePairConsumer_t consumer, void* state)
{
  ...
  while (*curRead && !consumerStopRequested) {
    ...
    while (strchr(whitespace, *(keyEnd - 1)))
      --keyEnd;
    while (strchr(whitespace, *(valEnd - 1)))
      --valEnd;
...
typedef struct nvtxwInterfaceCore_v1 {
  ...
  nvtxwResultCode_t (*SessionEnd)(nvtxwSessionHandle_t session);
  ...
  nvtxwResultCode_t (*StreamClose)(nvtxwStreamHandle_t stream);
