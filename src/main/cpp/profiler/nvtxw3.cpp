...
static void StripTrailingSlashes(char* path)
{
  ...
  if (newPathEnd != pathEnd) { *newPathEnd = '\0'; }
}
...
static char* AbsolutePath(char const* path)
{
  ...
  return path[0] == pathSep ? MakeHeapString(path)  // Absolute already
                                     ...
                                     char const* valEnd)
{
  ...
  if (!state->modeFound && keyLen == keyModeLen && strncmp(keyBegin, keyMode, keyLen) == 0) {
    int mode;
    char* val;
  ...
  if (!state->modeStringFound && keyLen == keyModeStringLen &&
...
static int GetInitModeFromConfig(char const* config, int* mode, char** modeString)
{
  ...
  *modeString = NULL;
  ...
  if (state.mode != NVTXW3_INIT_MODE_SEARCH_DEFAULT && !state.modeStringFound) { return 2; }
  ...
  void** moduleHandle)                   /* optional */
{
  ...
  *getInterfaceFunc = NULL;
  ...
  if (!filename) { return NVTXW3_RESULT_INVALID_ARGUMENT; }

  hModule = NVTXW3_DLLOPEN(filename);
  if (!hModule) { return NVTXW3_RESULT_LIBRARY_NOT_FOUND; }

  pfnLoadImplementation =
    (nvtxwLoadImplementation_t)NVTXW3_DLLFUNC(hModule, "nvtxwLoadImplementation");
  if (!pfnLoadImplementation) {
    NVTXW3_DLLCLOSE(hModule);
    return NVTXW3_RESULT_LOADER_SYMBOL_MISSING;
  }

  if (configString) { configSimple = SimplifyConfigHeapString(MakeHeapString(configString)); }
  ...
  void** moduleHandle)                   /* optional */
{
  ...
  free(filename);
  if (result == NVTXW3_RESULT_SUCCESS) { return NVTXW3_RESULT_SUCCESS; }

  /* 2. Standard search paths for dynamic libraries */
  result =
    InitLibraryFilename(NVTXW3_LIB_FILENAME_DEFAULT, configString, getInterfaceFunc, moduleHandle);
  if (result == NVTXW3_RESULT_SUCCESS) { return NVTXW3_RESULT_SUCCESS; }
  ...
  void** moduleHandle)                   /* optional */
{
  ...
  filename = AppendToPathHeapString(MakeHeapString(directory), NVTXW3_LIB_FILENAME_DEFAULT);
                                          ...
                                          void** moduleHandle)
{
  ...
  switch (mode) {
    case NVTXW3_INIT_MODE_SEARCH_DEFAULT:
      result = InitSearchDefault(config, getInterfaceFunc, moduleHandle);
      break;
    case NVTXW3_INIT_MODE_LIBRARY_FILENAME:
      result = InitLibraryFilename(modeString, config, getInterfaceFunc, moduleHandle);
      break;
    case NVTXW3_INIT_MODE_LIBRARY_DIRECTORY:
      result = InitLibraryDirectory(modeString, config, getInterfaceFunc, moduleHandle);
      break;
                                             ...
                                             void** moduleHandle)
{
  ...
  configFilename =
    AppendToPathHeapString(MakeHeapString(configDirectory), NVTXW3_CONFIG_FILENAME_DEFAULT);
                                                  ...
                                                  void** moduleHandle)
{
  ...
  if (!getInterfaceFunc) { return NVTXW3_RESULT_INVALID_ARGUMENT; }

  switch (mode) {
    case NVTXW3_INIT_MODE_SEARCH_DEFAULT:
      return InitSearchDefault(NULL, getInterfaceFunc, moduleHandle);
    case NVTXW3_INIT_MODE_LIBRARY_FILENAME:
      return InitLibraryFilename(modeString, NULL, getInterfaceFunc, moduleHandle);
    case NVTXW3_INIT_MODE_LIBRARY_DIRECTORY:
      return InitLibraryDirectory(modeString, NULL, getInterfaceFunc, moduleHandle);
    case NVTXW3_INIT_MODE_CONFIG_FILENAME:
      return InitConfigFilename(modeString, getInterfaceFunc, moduleHandle);
    case NVTXW3_INIT_MODE_CONFIG_DIRECTORY:
      return InitConfigDirectory(modeString, getInterfaceFunc, moduleHandle);
    case NVTXW3_INIT_MODE_CONFIG_STRING:
      return InitConfigString(modeString, getInterfaceFunc, moduleHandle);
    case NVTXW3_INIT_MODE_CONFIG_ENV_VAR:
      return InitConfigEnvVar(modeString, getInterfaceFunc, moduleHandle);
  }

  return NVTXW3_RESULT_INVALID_INIT_MODE;
}

NVTXW3_DECLSPEC void nvtxwUnload(void* moduleHandle)
{
