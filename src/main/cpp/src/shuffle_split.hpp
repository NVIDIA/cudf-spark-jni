 ...
 */
struct buffer_slice {
  uint8_t* data;  // pointer into the shared allocation
  size_t size;    // size of this slice
  size_t offset;  // offset from the beginning of the shared allocation

  buffer_slice() : data(nullptr), size(0), offset(0) {}
  buffer_slice(uint8_t* ptr, size_t sz, size_t off) : data(ptr), size(sz), offset(off) {}
};
...
/**
 * @brief Reassembles a set of partitions generated by shuffle_split using shared buffer approach.
 *
