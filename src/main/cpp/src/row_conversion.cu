                         ...
                         rmm::cuda_stream_view stream)
{
  ...
  thrust::uninitialized_fill(rmm::exec_policy(stream, cudf::get_current_device_resource_ref()),
                             d_row_sizes.begin(),
                             d_row_sizes.end(),
                             0);
                         ...
                         rmm::device_async_resource_ref mr)
{
  auto const total_size =
    thrust::reduce(rmm::exec_policy(stream, cudf::get_current_device_resource_ref()),
                   row_sizes,
                   row_sizes + num_rows);
  ...
  thrust::inclusive_scan(rmm::exec_policy(stream, cudf::get_current_device_resource_ref()),
                         row_sizes,
                         row_sizes + num_rows,
  ...
  while (last_row_end < num_rows) {
    ...
    auto const ub =
      thrust::upper_bound(rmm::exec_policy(stream, cudf::get_current_device_resource_ref()),
                          search_start,
                          search_end,
                          MAX_BATCH_SIZE);
                        ...
                        rmm::cuda_stream_view stream)
{
  ...
  return thrust::reduce(rmm::exec_policy(stream, cudf::get_current_device_resource_ref()),
                        num_tiles.begin(),
                        num_tiles.end());
}
                                         ...
                                         rmm::device_async_resource_ref mr)
{
  ...
  for (auto i : schema) {
    ...
    if (i.id() == type_id::STRING) {
      ...
      auto length_col =
        make_col(int32type, num_rows, false, stream, rmm::mr::get_current_device_resource_ref());
