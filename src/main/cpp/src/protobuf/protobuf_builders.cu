  ...
  bool propagate_invalid_enum_rows)
{
  ...
  auto [struct_mask, struct_null_count] =
    make_null_mask_from_parent_locations(d_parent_locs.data(), num_rows, stream, mr);
  return cudf::make_structs_column(
  ...
  bool propagate_invalid_enum_rows)
{
  ...
  if (total_count == 0) {
    rmm::device_uvector<int32_t> offsets(num_parent_rows + 1, stream, mr);
    thrust::fill(rmm::exec_policy_nosync(stream, mr), offsets.begin(), offsets.end(), 0);
    auto offsets_col = std::make_unique<cudf::column>(cudf::data_type{cudf::type_id::INT32},
                                                      num_parent_rows + 1,
                                                      offsets.release(),
    ...
    return make_list_column_with_parent_nulls(
      num_parent_rows, std::move(offsets_col), std::move(child_col), parent_locs, stream, mr);
  }

  auto list_offsets =
    make_list_offsets_from_counts(d_rep_counts, total_count, num_parent_rows, stream, mr);
  ...
  nested_repeated_location_provider loc_provider{
    row_offsets, base_offset, parent_locs, d_occurrences.data()};
  ...
  auto offsets_col = std::make_unique<cudf::column>(cudf::data_type{cudf::type_id::INT32},
                                                    ...
                                                    0);
  return make_list_column_with_parent_nulls(
    num_parent_rows, std::move(offsets_col), std::move(child_values), parent_locs, stream, mr);
}
